...
impl Session {
    pub(super) async fn reconstruct_history_from_rollout(
        ...
        let active_branch_items = rollout_items
            .iter()
            .any(|item| matches!(item, RolloutItem::EventMsg(EventMsg::ThreadNavigated(_))))
            .then(|| codex_rollout::active_branch_items(rollout_items));
        let rollout_items = active_branch_items.as_deref().unwrap_or(rollout_items);
