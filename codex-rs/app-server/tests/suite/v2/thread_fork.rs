...
async fn thread_fork_side_conversation_is_persisted_and_listed() -> Result<()> {
    ...
    let list_id = mcp
        .send_thread_list_request(ThreadListParams {
            ...
            search_term: None,
            side_parent_thread_id: Some(conversation_id.clone()),
        })
        .await?;
...
#[tokio::test]
async fn thread_fork_side_conversation_rejects_unknown_parent_turn() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), &server.uri())?;
    ...
    let fork_id = mcp
        .send_thread_fork_request(ThreadForkParams {
            ...
            side_conversation: Some(ThreadForkSideConversationParams {
                parent_turn_id: Some("missing-turn".to_string()),
            }),
            ..Default::default()
        })
        .await?;
